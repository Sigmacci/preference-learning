...
def Accuracy(output: torch.FloatTensor, target: torch.LongTensor) -> torch.FloatTensor:
    ...
    """
    return torch.FloatTensor([(target == (output > 0) * 1).detach().numpy().mean()])

def F1_Score(output: torch.FloatTensor, target: torch.LongTensor) -> torch.FloatTensor:
    ...
    return torch.FloatTensor([f1_score(target.detach().numpy(), (output > 0).detach().numpy())])
...
def AUC(output: torch.FloatTensor, target: torch.LongTensor) -> torch.FloatTensor:
    ...
    """
    return torch.FloatTensor([roc_auc_score(target.detach().numpy(), output.detach().numpy())])
...
class ScoreTracker:
    ...
    def append(self, loss: float, auc: float, acc: float, f1: float) -> None:
        ...
        Args:
            ...
            f1 (float): The F1 score to append.
        """
    ...
    def add(self, outputs: torch.FloatTensor, labels: torch.LongTensor) -> None:
        ...
        self.f1_scores.append(F1_Score(outputs, labels).item())
